...
for(const p of data.places){map+=`## ${p.name}\n\nObject: ${p.object}. Position on the shared world artwork (x/y, 0 to 1): ${p.anchor.join(', ')}. Clickable bounds (left/top/right/bottom): ${p.hitbox.join(', ')}.\n\n| Entry | Role or study | Organisation | Dates | Source |\n|---|---|---|---|---|\n`;for(const r of data.records.filter(r=>r.place===p.id))map+=`| ${r.id} | ${r.title} | ${r.organisation} | ${r.dates} | ${r.sources.map(s=>`${s.id}${s.page ? ', p. '+s.page : ''}: ${s.section}`).join('; ')} |\n`;map+='\n';}
